...
export class CityService {
  ...
  async processCitySeeder(file: Express.Multer.File) {
    try {
        ...
        try {
          const cityPromises = batch.map(async row => {
            ...
            const cityData = {
              ...
              stateId: stateId, // Using verified stateId
              countryId: countryId, // Using verified countryId
              active: false,
              isDeleted: false
            };
            ...
            if(foundCity) {
              await this.cityRepository.updateWithFilter(
                { 
                  name: cityData.name,
                  countryId: cityData.countryId,
                  isDeleted: false 
      ...
      throw new Error(`Failed to process city seeder: ${error.message}`);
    }
  }
}
