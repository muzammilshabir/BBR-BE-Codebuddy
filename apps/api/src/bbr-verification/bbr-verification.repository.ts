...
import { BbrVerification } from './schema/bbr-verification.schema';
...
export class BbrVerificationRepository extends BaseRepository<BbrVerification> {
  constructor(
    ...
    private readonly bbrVerificationModel: Model<BbrVerification>
  ) {
    super(bbrVerificationModel);
  }

  async findAllVerifications(listBbrVerificationDto: ListBbrVerificationDto) {
    const { search, status, residenceId, paymentStatus, verificationType } = listBbrVerificationDto;
    const paginationOptions = PaginationService.prepareOptions(listBbrVerificationDto);
      ...
      pipeline.push({
        $match: {
          $or: [
            ...
            { 'verificationInsight': { $regex: search, $options: 'i' } },
          ],
        },
      });
    }
  ...
  async hasActiveOrPendingRequest(
    ...
    const count = await this.bbrVerificationModel.countDocuments({
      ...
      $or: [
        ...
        {
          status: BbrVerificationStatus.APPROVED,
        },
      ],
    });
    return count > 0;
