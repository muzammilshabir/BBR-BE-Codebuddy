...
export class FeatureRequestRepository extends BaseRepository<FeatureRequest> {
  ...
  async findAllFeatureRequest(listFeatureRequestDto: ListFeatureRequestDto) {
    const { search, status, residenceId, paymentStatus } = listFeatureRequestDto;
    const paginationOptions = PaginationService.prepareOptions(listFeatureRequestDto);
